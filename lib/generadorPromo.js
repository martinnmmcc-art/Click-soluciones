...
export function textoFacebook(producto, { etiqueta = "NOVEDAD" } = {}) {
  ...
  m += `🛒 Comprá online: ${NEGOCIO_INFO.web}\n`;
...
export async function generarPlacaMultiple(productos, { titulo = "OFERTAS" } = {}) {
  ...
  ctx.fillText(NEGOCIO.nombre, LADO / 2, 62);

  ctx.font = "bold 30px system-ui, sans-serif";
  const anchoEt = ctx.measureText(titulo).width + 60;
  ctx.fillStyle = NARANJA;
  ctx.beginPath();
  ctx.roundRect((LADO - anchoEt) / 2, 82, anchoEt, 54, 27);
  ctx.fill();
  ctx.fillStyle = "#FFFFFF";
  ctx.fillText(titulo, LADO / 2, 120);

  // Grilla de 2x2
  const margen = 30;
  const inicioY = 160;
  const anchoCelda = (LADO - margen * 3) / 2;
  const altoCelda = (LADO - inicioY - 150 - margen) / 2;
  ...
  for (let i = 0; i < lista.length; i++) {
    ...
    ctx.fillStyle = "#FFFFFF";
    ctx.beginPath();
    ctx.roundRect(x, y, anchoCelda, altoCelda, 22);
    ctx.fill();

    // Foto
    const altoFoto = altoCelda - 110;
    try {
      const img = await cargarImagen(p.imagen_url);
      const escala = Math.min((anchoCelda - 24) / img.width, (altoFoto - 16) / img.height);
      ...
      ctx.fillText("📦", x + anchoCelda / 2, y + altoFoto / 2 + 20);
    }

    // Nombre
    ctx.fillStyle = "#1F2937";
    ctx.font = "bold 21px system-ui, sans-serif";
    const lineas = repartirEnLineas(ctx, p.nombre, anchoCelda - 26, 2);
    ...
    if (enOferta) {
      ...
      ctx.fillText(tachado, x + anchoCelda / 2, y + altoCelda - 38);

      const anchoTach = ctx.measureText(tachado).width;
      ctx.strokeStyle = "#9CA3AF";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(x + anchoCelda / 2 - anchoTach / 2, y + altoCelda - 44);
      ctx.lineTo(x + anchoCelda / 2 + anchoTach / 2, y + altoCelda - 44);
      ctx.stroke();
    }

    ctx.fillStyle = enOferta ? "#DC2626" : "#1560D4";
    ctx.font = "bold 32px system-ui, sans-serif";
    ctx.fillText(formatearPrecio(precio), x + anchoCelda / 2, y + altoCelda - 12);
  }

  // Pie con contacto
  ctx.fillStyle = "rgba(255,255,255,0.14)";
  ctx.fillRect(0, LADO - 130, LADO, 130);

  ctx.fillStyle = "#FFFFFF";
  ctx.font = "bold 38px system-ui, sans-serif";
  ctx.fillText(NEGOCIO.telefono, LADO / 2, LADO - 78);

  ctx.font = "28px system-ui, sans-serif";
  ctx.fillStyle = "rgba(255,255,255,0.92)";
  ctx.fillText(NEGOCIO.web, LADO / 2, LADO - 42);

  ctx.font = "22px system-ui, sans-serif";
  ctx.fillStyle = "rgba(255,255,255,0.72)";
  ctx.fillText(`Envíos en ${NEGOCIO.zona}`, LADO / 2, LADO - 14);
...
export function textoMultiple(productos, { titulo = "OFERTAS" } = {}) {
  ...
  m += `🛒 Todo el catálogo: ${NEGOCIO.web}\n`;
