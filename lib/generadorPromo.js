...
export function textoWhatsApp(producto, { etiqueta = "NOVEDAD" } = {}) {
  const { enOferta, precio } = datosPrecio(producto);
  const escasez = textoEscasez(producto);

  // Corto a propósito: en un estado la imagen ya muestra el producto, el
  // precio y la escasez. El texto solo tiene que dar el dato de contacto,
  // porque en WhatsApp no se puede tocar un enlace desde la imagen.
  let m = `*${producto.nombre}*\n`;
  m += `${enOferta ? "🔥 " : ""}$${precio.toLocaleString("es-AR")}\n`;

  if (escasez) m += `${escasez.replace(/\*/g, "")}\n`;

  m += `\n📲 ${NEGOCIO_INFO.telefono}\n`;
  m += `🛒 ${NEGOCIO_INFO.web}/?de=wa`;

  return m;
}

// Versión más larga, para cuando se lo mandás directo a un cliente por chat
// en vez de publicarlo como estado.
export function textoWhatsAppLargo(producto, { etiqueta = "NOVEDAD" } = {}) {
...
export function textoFacebook(producto, { etiqueta = "NOVEDAD" } = {}) {
  ...
  if (enOferta) {
    ...
    m += `🔥 AHORA $${precio.toLocaleString("es-AR")}\n`;
    m += `Te ahorrás $${ahorro.toLocaleString("es-AR")}\n\n`;
  } else {
    m += `💰 $${precio.toLocaleString("es-AR")}\n\n`;
  }
...
export async function generarPlacaMultiple(
  ...
  for (let i = 0; i < lista.length; i++) {
    ...
    // Repartimos la celda en zonas fijas de arriba hacia abajo: foto,
    // nombre y precios. Antes el nombre de dos líneas empujaba hacia abajo
    // y el precio quedaba fuera de la tarjeta.
    const altoPrecios = enOferta ? 78 : 52;
    const altoNombre = vertical ? 62 : 54;
    const altoFoto = altoCelda - altoNombre - altoPrecios - 24;

    try {
      const img = await cargarImagen(p.imagen_url);
      const escala = Math.min(
        (anchoCelda - 30) / img.width,
        (altoFoto - 10) / img.height
      );
      const w = img.width * escala;
      const h = img.height * escala;
      ctx.drawImage(img, x + (anchoCelda - w) / 2, y + 12 + (altoFoto - h) / 2, w, h);

      // Marca de agua sobre cada foto. Acá también hace falta: si alguien
      // recorta un producto de la placa, la foto suelta sigue diciendo de
      // dónde salió y es más difícil rastrear al proveedor.
      ctx.save();
      ctx.beginPath();
      ctx.rect(x + 6, y + 6, anchoCelda - 12, altoFoto);
      ctx.clip();

      ctx.translate(x + anchoCelda / 2, y + altoFoto / 2);
      ctx.rotate(-Math.PI / 6);
      ctx.font = `bold ${vertical ? 19 : 16}px system-ui, sans-serif`;
      ctx.textAlign = "center";
      ctx.fillStyle = "#0E3F91";

      const pasoY = vertical ? 72 : 62;
      const pasoX = vertical ? 210 : 180;

      for (let fila = -3; fila <= 3; fila++) {
        for (let col = -2; col <= 2; col++) {
          const mx = col * pasoX + (fila % 2 ? pasoX / 2 : 0);
          const my = fila * pasoY;
          // Más tenue en el centro, para no tapar el producto
          const dist = Math.sqrt(mx * mx + my * my);
          ctx.globalAlpha = dist < 90 ? 0.09 : 0.17;
          ctx.fillText(NEGOCIO.web, mx, my);
        }
      }
      ctx.restore();
    } catch (e) {
    ...
    // Etiqueta de oferta, sin porcentaje: los descuentos reales dan números
    // raros (16,5%) y mostrarlos redondeados sería inexacto.
    if (enOferta) {
      ctx.fillStyle = "#DC2626";
      ctx.beginPath();
      ctx.roundRect(x + 12, y + 12, 96, 38, 19);
      ctx.fill();
      ctx.fillStyle = "#FFFFFF";
      ctx.font = "bold 21px system-ui, sans-serif";
      ctx.fillText("OFERTA", x + 60, y + 38);
    }

    // Nombre: arranca justo después de la foto, con dos líneas como máximo
    ctx.fillStyle = "#1F2937";
    ctx.font = `bold ${vertical ? 23 : 20}px system-ui, sans-serif`;
    const lineas = repartirEnLineas(ctx, p.nombre, anchoCelda - 30, 2);
    let ty = y + altoFoto + (vertical ? 32 : 28);
    lineas.forEach((linea) => {
      ctx.fillText(linea, x + anchoCelda / 2, ty);
      ty += vertical ? 27 : 24;
    });

    // Precios: siempre en la base de la tarjeta, sin depender del nombre
    const precio = enOferta ? p.precio_oferta : p.precio;
    const baseY = y + altoCelda - 18;

    if (enOferta) {
      ctx.fillStyle = "#9CA3AF";
      ctx.font = `${vertical ? 23 : 20}px system-ui, sans-serif`;
      const tachado = formatearPrecio(p.precio);
      const yTach = baseY - (vertical ? 42 : 36);
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(x + anchoCelda / 2 - anchoTach / 2 - 4, yTach - 7);
      ctx.lineTo(x + anchoCelda / 2 + anchoTach / 2 + 4, yTach - 7);
      ctx.stroke();
    }

    ctx.fillStyle = enOferta ? "#DC2626" : "#1560D4";
    ctx.font = `bold ${vertical ? 40 : 34}px system-ui, sans-serif`;
    ctx.fillText(formatearPrecio(precio), x + anchoCelda / 2, baseY);
  }
...
export function textoMultiple(productos, { titulo = "OFERTAS" } = {}) {
  ...
  lista.forEach((p) => {
    const enOferta = p.precio_oferta && Number(p.precio_oferta) < Number(p.precio);
    const precio = enOferta ? p.precio_oferta : p.precio;
    m += `▸ ${p.nombre} — $${Number(precio).toLocaleString("es-AR")}\n`;
  });

  m += `\n📲 ${NEGOCIO.telefono}\n`;
  m += `🛒 ${NEGOCIO.web}/?de=promo`;
