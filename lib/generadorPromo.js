...
export async function generarPlaca(producto, { etiqueta = "NOVEDAD" } = {}) {
  ...
  try {
    ...
    // Marca de agua sobre la foto: si alguien reenvía la imagen suelta,
    // igual queda claro de dónde salió.
    ctx.save();
    ctx.globalAlpha = 0.55;
    ctx.font = "bold 30px system-ui, sans-serif";
    ctx.textAlign = "right";

    const marca = NEGOCIO.web;
    const anchoMarca = ctx.measureText(marca).width;
    const xMarca = ANCHO - 80;
    const yMarca = cajaY + cajaAlto - 34;

    ctx.fillStyle = "rgba(0,0,0,0.45)";
    ctx.beginPath();
    ctx.roundRect(xMarca - anchoMarca - 22, yMarca - 30, anchoMarca + 32, 44, 22);
    ctx.fill();

    ctx.globalAlpha = 0.95;
    ctx.fillStyle = "#FFFFFF";
    ctx.fillText(marca, xMarca, yMarca);
    ctx.restore();
    ctx.textAlign = "center";
  } catch (e) {
