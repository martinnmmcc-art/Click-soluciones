...
// Una venta queda CERRADA cuando ya se entregó y está pagada. Si el cliente
// pagó de más ("A favor"), también se cierra: la plata a favor no se pierde,
// queda sumada en el saldo del cliente (Resumen por cliente) para su
// próxima compra.
export const PAGOS_QUE_CIERRAN = ["pagado", "a_favor"];

export function ventaCerrada(pedido) {
  return pedido?.estado === "entregado" && PAGOS_QUE_CIERRAN.includes(pedido?.estado_pago);
}
...
// El texto del pago depende de si el pedido ya se entregó o no. Decirle
// "pagalo al recibirlo" a alguien que ya lo tiene en la mano no tiene
// sentido y hace que no sepa qué hacer.
export function textoPago(estadoPagoClave, estadoEntregaClave, saldo = 0) {
  const yaLoTiene =
    estadoEntregaClave === "entregado" || estadoEntregaClave === "listo";

  if (estadoPagoClave === "pagado") return "¡Listo, recibimos tu pago!";
  if (estadoPagoClave === "comprobando")
    return "Recibimos tu comprobante, lo estamos verificando";
  if (estadoPagoClave === "a_favor")
    return "Pagaste de más, lo descontamos de la próxima";

  if (estadoPagoClave === "senado" || estadoPagoClave === "deuda_parcial") {
    return yaLoTiene
      ? "Ya recibiste el pedido. Te queda un saldo por pagar."
      : "Recibimos una parte. El resto lo podés pagar al recibirlo.";
  }

  // falta_pagar
  if (estadoEntregaClave === "entregado")
    return "Ya recibiste el pedido. Te queda pendiente el pago.";
  if (estadoEntregaClave === "listo")
    return "Está listo para retirar. Podés transferir antes o pagar al retirarlo.";
  if (estadoEntregaClave === "repartiendo")
    return "Va en camino. Podés transferir ahora o pagar al recibirlo.";
  if (estadoEntregaClave === "cancelado" || estadoEntregaClave === "rechazado")
    return "";

  return "Podés transferir ahora o pagar cuando te lo entreguemos.";
}
