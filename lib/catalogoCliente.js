...
export function guardarCatalogoCliente(productos) {
  ...
  // Solo guardamos los productos propios. Los "a pedido" son casi 2800 y
  // no tiene sentido tenerlos offline: igual hay que consultar antes de
  // encargarlos, y llenarían el celular del cliente al pedo.
  const propios = productos.filter((p) => !p.bajo_pedido);
