...
export function normalizarVideoUrl(url) {
  ...
  // Facebook: se arma el link limpio del video y se usa el reproductor de
  // Facebook. Los links "share" y "fb.watch" primero hay que abrirlos en el
  // servidor (ver esLinkCorto); si no se pudieron abrir, quedan sin
  // convertir y el formulario avisa que no se pueden reproducir.
  if (/facebook\.com\/plugins\/video\.php/i.test(u)) return u;
  if (/(facebook\.com|fb\.watch)\//i.test(u)) {
    let limpio = null;
    let vertical = false;
    const reel = u.match(/facebook\.com\/reel\/(\d+)/i);
    const watch = u.match(/facebook\.com\/watch\/?\?(?:.*&)?v=(\d+)/i);
    const videos = u.match(/facebook\.com\/([\w.]+)\/videos\/(?:[\w.-]+\/)?(\d+)/i);
    if (reel) {
      limpio = `https://www.facebook.com/reel/${reel[1]}/`;
      vertical = true;
    } else if (watch) {
      limpio = `https://www.facebook.com/watch/?v=${watch[1]}`;
    } else if (videos) {
      limpio = `https://www.facebook.com/${videos[1]}/videos/${videos[2]}/`;
    }
    if (!limpio) return u;
